...
import { Check, ChevronLeft, ChevronRight, CircleDot, Clock3, FilePlus2, LockKeyhole, Timer } from 'lucide-react';
  ...
  return (
    <section className="flex min-h-[370px] h-full flex-col" aria-label="My Ticket Status Overview">
      ...
      <motion.div key={activeStep} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.22 }} className="flex flex-1 flex-col justify-between rounded-xl border border-token bg-app p-4">
        <div><div className="mb-4 flex items-center gap-2" style={{ color: current.color }}>{current.icon}<span className="text-[10px] font-mono uppercase tracking-wide">Status {activeStep + 1}</span></div><div className="flex items-end gap-2"><span className="text-4xl font-bold text-primary">{current.count}</span><span className="mb-1 text-xs text-secondary">{current.label}</span></div><p className="mt-3 text-[11px] leading-relaxed text-tertiary">{current.detail}</p></div>
