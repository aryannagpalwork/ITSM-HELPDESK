...
export const AdminPendingUsers: React.FC = () => {
  ...
  if (!isAuthorized) {
    return (
      <div className="flex-1 bg-app p-4 sm:p-6 lg:p-8 flex flex-col items-center justify-center h-screen font-sans">
        ...
        <p className="text-xs text-secondary mt-2 max-w-sm text-center leading-relaxed">
          Access denied. This console requires **Administrator** authorization credentials.
        </p>
        <div className="mt-8 p-4 bg-card border border-token rounded-xl max-w-sm text-center">
          <p className="text-[10px] text-secondary font-mono">DEMO SHORTCUT</p>
          <div className="mt-3.5 w-full py-2 bg-input border border-token rounded-lg text-xs font-semibold text-secondary flex items-center justify-center space-x-2">
  ...
  return (
    <div className="flex-1 bg-app p-4 sm:p-6 lg:p-8 overflow-y-auto h-screen font-sans">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        ...
        <div className="flex items-center space-x-3 text-xs">
          ...
          <button
            ...
            className="p-2 bg-card border border-token rounded-lg hover:bg-hover transition-colors"
            title="Refresh"
          >
            <RefreshCw className="w-4 h-4 text-secondary" />
          </button>
          <div className="p-2 bg-card border border-token rounded-lg text-secondary font-mono flex items-center gap-2">
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-xs font-medium transition-all whitespace-nowrap ${
                activeTab === tab.key
                  ? 'bg-card text-primary border border-token shadow-sm'
                  : 'text-secondary hover:text-primary hover:bg-hover border border-transparent'
              }`}
            >
              <Icon className={`w-3.5 h-3.5 ${activeTab === tab.key ? 'text-accent' : 'text-tertiary'}`} />
              <span>{tab.label}</span>
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full input-token rounded-xl pl-10 pr-4 py-2.5 text-xs outline-none placeholder-zinc-500"
        />
      </div>
                  ...
                  <tr key={user.id} className="hover:bg-hover transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-accent-soft border border-accent/20 rounded-lg shrink-0">
      ...
      {modal.type && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-card-solid border border-token rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
            {/* Create Modal */}
            {modal.type === 'create' && (
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button onClick={closeModal} className="p-1 hover:bg-hover rounded-lg transition-colors">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full input-token rounded-lg px-3 py-2 text-xs outline-none placeholder-zinc-500"
                ...
                <div className="flex justify-end gap-2 mt-6">
                  <button onClick={closeModal} className="px-4 py-2 bg-input hover:bg-hover text-secondary rounded-lg text-[10px] font-semibold transition-all">
                    Cancel
                  </button>
                  <button
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={formEmail}
                      readOnly
                      className="w-full bg-input border border-token rounded-lg px-3 py-2 text-xs text-tertiary outline-none cursor-not-allowed"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-mono uppercase text-tertiary mb-1.5">Role</label>
            ...
            {modal.type === 'delete' && modal.user && (
              <div className="p-6">
                ...
                <div className="p-4 bg-input border border-token rounded-xl mb-4">
                  <p className="text-xs text-primary font-semibold">{modal.user.name}</p>
                  <p className="text-[10px] text-secondary">{modal.user.email}</p>
                </div>
