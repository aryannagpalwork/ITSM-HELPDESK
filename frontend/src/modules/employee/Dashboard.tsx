...
export const EmployeeDashboard: React.FC = () => {
  ...
  const chartTickets = useMemo(() => myTickets.filter(ticket => {
    ...
    if (chartFilter === 'open') return ticket.status === 'open' || ticket.status === 'in_progress' || ticket.status === 'awaiting_user_response';
    return true;
                    ...
                    <div className="min-w-0 flex-1 pr-4">
                      <div className="flex items-center space-x-2.5 mb-1.5 flex-wrap gap-y-1">
                        ...
                        <span className={`text-[9px] font-mono uppercase px-2 py-0.5 rounded-full font-semibold ${getStatusBadgeColor(ticket.status)}`}>
                          {ticket.status === 'in_progress' ? 'In Progress' : ticket.status === 'awaiting_user_response' ? 'Awaiting User Response' : ticket.status}
                        </span>
                        {ticket.category && (
