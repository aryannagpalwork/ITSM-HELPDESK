...
export const AgentDashboard: React.FC = () => {
  ...
  const m: AgentMetrics & { fcrRate: number | null; csatScore: number | null } = {
    ...
    fcrRate: roleAgentKPIs?.fcrRate ?? null,
    csatScore: roleAgentKPIs?.csatScore ?? null,
    ...
    >
      ...
      <div className="mb-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          <KpiCard tokens={tokens} label="Assigned" value={m.assignedTickets} icon={<Inbox className="w-3 h-3" style={{ color: tokens.accentPrimary }} />} color="" subtitle={metricsAvailable ? 'Click to focus charts on your tickets' : 'Loading...'} isActive={cardFilter?.label === 'Assigned'} onClick={() => toggleCardFilter({ label: 'Assigned', assignment: 'assigned' })} onOpen={() => navigate('/agent/tickets')} openLabel="Open full ticket queue" />
          <KpiCard tokens={tokens} label="Resolved Today" value={m.resolvedToday} icon={<CheckCircle2 className="w-3 h-3" style={{ color: tokens.statusInfo }} />} color="" subtitle={metricsAvailable ? 'Click to focus charts on resolved' : 'Loading...'} isActive={cardFilter?.label === 'Resolved Today'} onClick={() => toggleCardFilter({ label: 'Resolved Today', statuses: ['resolved', 'closed'] })} onOpen={() => navigate('/agent/tickets?status=resolved')} openLabel="Open resolved tickets" />
          <KpiCard tokens={tokens} label="Mean Time To Resolution" value={metricsAvailable ? `${m.avgResolutionTimeHours}h` : 'N/A'} icon={<Timer className="w-3 h-3" style={{ color: tokens.statusInfo }} />} color="" subtitle="Avg hours to resolve" isActive={cardFilter?.label === 'Mean Time To Resolution'} onClick={() => toggleCardFilter({ label: 'Mean Time To Resolution', statuses: ['resolved', 'closed'] })} onOpen={() => navigate('/agent/tickets?status=resolved')} openLabel="Open resolved tickets" />
          <KpiCard tokens={tokens} label="First Contact Resolution" value={richKpisAvailable && m.fcrRate !== null ? `${m.fcrRate}%` : '—'} icon={<CheckCheck className="w-3 h-3" style={{ color: tokens.statusSuccess }} />} color="" subtitle="Solved on first contact" accent={richKpisAvailable} isActive={cardFilter?.label === 'First Contact Resolution'} onClick={() => toggleCardFilter({ label: 'First Contact Resolution', statuses: ['resolved', 'closed'] })} onOpen={() => navigate('/agent/tickets?status=resolved')} openLabel="Open resolved tickets" />
          <KpiCard tokens={tokens} label="User Satisfaction (CSAT)" value={richKpisAvailable && m.csatScore !== null ? `${m.csatScore}%` : '—'} icon={<ThumbsUp className="w-3 h-3" style={{ color: tokens.statusSuccess }} />} color="" subtitle="Quality-derived score" accent={richKpisAvailable} isActive={cardFilter?.label === 'User Satisfaction (CSAT)'} onClick={() => toggleCardFilter({ label: 'User Satisfaction (CSAT)', statuses: ['resolved', 'closed'] })} onOpen={() => navigate('/agent/tickets?status=resolved')} openLabel="Open resolved tickets" />
        </div>
      </div>
